...
At this stage, it is very important to understant when we interract with the Secret Store node (over an HTTP API and port 8010) and when we use the HTTP RPC API from Alice's node (with the port 8545).
...
- `0x45ce99addb0f8385bd24f30da619ddcc0cadadab73e2a4ffb7801083086b3fc2` is the `Document key id` we chosed.
...
- `1` is the threshold chosed. We will allow the generation of the key if 2 out of 3 nodes are up and running.
...
The result are the `encrypted_key` and pair (`common_point`,  `encrypted_point`) that will be used to encrypt the Document (offline).
```json
{
  "jsonrpc": "2.0",
  "result": {
...
Alice has encrypted her document, now she needs to store her key in the Secret Store for others to be able to access it if she decides to do so.
...
-   `45ce99a..3086b3fc2`: is the `Document key id` we chose at the beginning of this section;
-   `5fcf162..bd3ac0400`: is the signed server key id from [step 1.1](#1-get-the-server-key-for-this-document);
...
[ < Previous Step (Configuring each node)](Secret-Store-Tutorial-1) | [ Next Step (Bob retrieves the document's key) > ](Secret-Store-Tutorial-1)
